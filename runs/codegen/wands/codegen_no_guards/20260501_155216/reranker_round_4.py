def rerank_wands(*args, **kwargs):
    ...
    for did in candidate_ids:
        ...
        # Color matches: add positive boosts; avoid negative penalties to not suppress color-intent queries
        if q_colors:
            for c in q_colors:
        ...
        if q_products or q_phrase_hits:
            present_word = any((f" {p} " in f" {title} ") or (f" {p} " in f" {desc} ") for p in q_products)
            present_phrase = any((p in title) or (p in desc) for p in q_phrase_hits)
