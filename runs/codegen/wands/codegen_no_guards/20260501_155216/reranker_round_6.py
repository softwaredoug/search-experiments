def rerank_wands(*args, **kwargs):
    ...
    w_desc_or = 0.9
    w_desc_and = 1.0
    w_var_title_and = 1.6 if has_dims else 1.2
    w_var_title_or = 0.6
    w_emb = 1.1
    ...
    color_terms = {"black","white","gray","grey","blue","navy","red","green","pink","purple","gold","silver","bronze","brass","brown","beige","ivory","charcoal","champagne"}
    ...
    for did in candidate_ids:
        ...
        if title:
            ...
            if q_lower in title:
                score += 0.4
            # Token coverage boost using content terms (ignore stopwords/numbers)
            if q_content_terms and all(t in title for t in q_content_terms):
                score += 0.8
            else:
                if q_content_terms:
                    overlap = sum(1 for t in set(q_content_terms) if t in title)
                    score += 0.15 * (overlap / max(1, len(set(q_content_terms))))
        else:
        ...
        # Color matches: add positive boosts; avoid false-positives from brand names and inside words
        if q_colors:
            def has_color(c, text):
                # avoid boosting brand phrase "canora grey" and ensure whole-word match
                if c == 'grey' and 'canora grey' in text:
                    return False
                return re.search(rf"\\b{re.escape(c)}\\b", text) is not None
            for c in q_colors:
                if has_color(c, title):
                    score += 0.2
                if has_color(c, desc):
                    score += 0.2
        ...
        # Targeted brand boost for queries with 'kohler'
        if q_has_kohler:
